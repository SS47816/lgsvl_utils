...
#include "detected_objects_visualizer.h"

DetectedObjectsVisualizer::DetectedObjectsVisualizer() : arrow_height_(0.5), label_height_(1.0)
{
  ...
  subscriber_detected_objects_ =
    node_handle_.subscribe(object_src_topic, 1,
                           &DetectedObjectsVisualizer::DetectedObjectsCallback, this);
...
float DetectedObjectsVisualizer::CheckColor(double value)
{
  float final_value;
  if (value > 255.)
    final_value = 1.f;
...
float DetectedObjectsVisualizer::CheckAlpha(double value)
{
  float final_value;
  if (value > 1.)
    final_value = 1.f;
...
void DetectedObjectsVisualizer::DetectedObjectsCallback(const autoware_msgs::DetectedObjectArray &in_objects)
{
...
DetectedObjectsVisualizer::ObjectsToCentroids(const autoware_msgs::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
DetectedObjectsVisualizer::ObjectsToBoxes(const autoware_msgs::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
DetectedObjectsVisualizer::ObjectsToHulls(const autoware_msgs::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
DetectedObjectsVisualizer::ObjectsToArrows(const autoware_msgs::DetectedObjectArray &in_objects)
{
...
visualization_msgs::MarkerArray
DetectedObjectsVisualizer::ObjectsToLabels(const autoware_msgs::DetectedObjectArray &in_objects)
{
...
bool DetectedObjectsVisualizer::IsObjectValid(const autoware_msgs::DetectedObject &in_object)
{
  if (!in_object.valid ||
